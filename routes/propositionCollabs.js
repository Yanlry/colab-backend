...
router.post("/propositions", (req, res) => {
  ...
  User.findOne({ username: cible }).then((cible) => {
    if (!cible) {
      return res.json({ result: false, error: "Utilisateur cible non trouvé" });
    }
    User.findOne({ username: initiateur }).then((initiateur) => {
      if (!initiateur) {
        return res.json({
          result: false,
          error: "Utilisateur initiateur non trouvé",
        });
      }
      Annonce.findOne({ token }).then((annonce) => {
        if (!annonce) {
          return res.json({ result: false, error: "Annonce non trouvée" });
        }
        const conversationId = `${annonce._id}_${initiateur._id}_${cible._id}`;

        PropositionCollab.findOne({
          annonce: annonce._id,
          ...
          if (existingProposition) {
            return res.json({
              result: false,
              message:
                "Vous avez déjà fait une demande de collaboration pour cette annonce.",
            });
          }
          ...
          proposition.save().then((savedProposition) => {
            res.json({
              result: true,
              message: "La demande de collaboration a été envoyée avec succès.",
...
router.post("/propositions/cible", (req, res) => {
  ...
  User.findOne({ token }).then((user) => {
    ...
    PropositionCollab.find({ cible: user._id })
      ...
      .select("_id annonce initiateur statut")
      .then((propositionsCible) => {
        console.log("Propositions Cible :", propositionsCible);
          ...
          if (!proposition.initiateur) {
            ...
            if (proposition.statut === "en_attente") {
              ...
              additionalMessage = `${
                proposition.annonce
                  ? proposition.annonce.title
                  : "L'annonce a étais supprimé"
              }.`;
            } else if (proposition.statut === "refusé") {
              additionalMessage = `${
                proposition.annonce
                  ? proposition.annonce.title
                  : `Vous avez supprimé l'annonce`
              }`;
            }
  ...
  User.findOne({ token })
    .then((user) => {
      ...
      PropositionCollab.find({ initiateur: user._id })
        ...
        .select("_id annonce cible statut")
        .then((propositionsInitiateur) => {
          console.log("Propositions Initiateur :", propositionsInitiateur);
              ...
              if (proposition.statut === "en_attente") {
                ...
                additionalMessage = `${
                  proposition.annonce
                    ? proposition.annonce.title
                    : "Vous avez supprimé l'annonce"
                }. Son numéro de téléphone a été ajouté à la liste de contacts.`;
              } else if (proposition.statut === "refusé") {
                additionalMessage = `${
                  proposition.annonce
                    ? proposition.annonce.title
                    : "Une annonce qui n'existe plus"
                }.`;
              }
  ...
  User.findOne({ token }).then((user) => {
    ...
    PropositionCollab.findOneAndUpdate(
      ...
      if (updatedProposition) {
        return res.json({
          result: true,
          message: "La proposition de collaboration a été acceptée avec succès",
        });
      }
      res.json({
        result: false,
        error: "Aucune proposition de collaboration en attente trouvée",
      });
    });
  });
});
  ...
  User.findOne({ token }).then((user) => {
    ...
    PropositionCollab.findOneAndUpdate(
      ...
      if (updatedProposition) {
        return res.json({
          result: true,
          message: "La proposition de collaboration a été refusée avec succès",
        });
      }
      res.json({
        result: false,
      ...
      .then((collaborations) => {
        ...
        collaborations.forEach((collaboration) => {
          ...
          if (
            contactUser &&
            contactUser._id &&
            !contactUser._id.equals(user._id)
          ) {
  ...
  PropositionCollab.findByIdAndDelete(propositionCollabsId)
    .then((deletedCollaboration) => {
      if (deletedCollaboration) {
        res.json({
          result: true,
          message: "La collaboration a été supprimée avec succès",
        });
      } else {
